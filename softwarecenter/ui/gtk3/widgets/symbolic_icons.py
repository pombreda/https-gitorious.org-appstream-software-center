...
class SymbolicIcon(Gtk.Image):
    ...
    def __init__(self, name, datadir_override=None):
        Gtk.Image.__init__(self)
        self.set_name("symbolic-icon")
        # get base dir
        SYMBOLIC_DIR = os.path.join(
            datadir_override or softwarecenter.paths.datadir,
            "ui/gtk3/art/icons/")
...
class PendingSymbolicIcon(SymbolicIcon, RotationAboutCenterAnimation):
    ...
    def __init__(self, name, datadir_override=None):
        SymbolicIcon.__init__(self, name, datadir_override)
...
if __name__ == "__main__":
    ...
    win.add(hb)
    ico = SymbolicIcon("available", os.path.join(os.getcwd(), 'data'))
    hb.add(ico)
    ico = PendingSymbolicIcon("pending", os.path.join(os.getcwd(), 'data'))
    ico.start()
